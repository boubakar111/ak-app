	...
	function load_system_info(){
			...
			if($qry){
				...
				while($row = $qry->fetch(PDO::FETCH_ASSOC)){
					// Stocker chaque champ de la base de données comme une propriété de l'objet
					$system_info->{$row['meta_field']} = $row['meta_value'];
				}
				// Retourner l'objet contenant les informations extraites
				return $system_info;
			} else {
	...
	function update_settings_info(){
		...
		foreach ($_POST as $key => $value) {
			...
			if(isset($_SESSION['system_info'][$key])){
				...
				$qry = $this->db->query("UPDATE system_info set meta_value = '{$value}' where meta_field = '{$key}' ");
			}else{
				$qry = $this->db->query("INSERT into system_info set meta_value = '{$value}', meta_field = '{$key}' ");
			}
		}
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			if(isset($uploaded_img) && $uploaded_img == true){
				if(isset($_SESSION['system_info']['logo'])){
					$qry = $this->db->query("UPDATE system_info set meta_value = '{$fname}' where meta_field = 'logo' ");
					if(is_file(base_app.$_SESSION['system_info']['logo'])) unlink(base_app.$_SESSION['system_info']['logo']);
				}else{
					$qry = $this->db->query("INSERT into system_info set meta_value = '{$fname}',meta_field = 'logo' ");
				}
				unset($uploaded_img);
			}
		}
		if(isset($_FILES['cover']) && $_FILES['cover']['tmp_name'] != ''){
			...
			if(isset($uploaded_img) && $uploaded_img == true){
				if(isset($_SESSION['system_info']['cover'])){
					$qry = $this->db->query("UPDATE system_info set meta_value = '{$fname}' where meta_field = 'cover' ");
					if(is_file(base_app.$_SESSION['system_info']['cover'])) unlink(base_app.$_SESSION['system_info']['cover']);
				}else{
					$qry = $this->db->query("INSERT into system_info set meta_value = '{$fname}',meta_field = 'cover' ");
				}
				unset($uploaded_img);
			}
		}
